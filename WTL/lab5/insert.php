...
<html lang="en">
   <head>
    ...
    <body onload="document.Valid.empid.focus()">
            ...
            $exist = false;
                if($_SERVER['REQUEST_METHOD']=='POST')
                {
                    ...
                    if(mysqli_num_rows($result)>0)
                        echo "<div id='alert_danger'>Employee ID already exists!<i class='fa fatimes' onclick='alert_dismiss2()'></i></div>";
                    
                    else
                    {
                        $sql = "INSERT INTO `employees` (`empid`, `empname`, `deptname`, `phno`, `jdate`)
                        VALUES ('$empid', '$name', '$dept', '$phone', '$date');";
                        $result = mysqli_query($conn,$sql);
                        if($result)
                            echo "<div id='alert_success'>Record inserted successfully!<i class='fa fatimes' onclick='alert_dismiss1()'></i></div>";
                        else
        ...
        <div>
            <form name="Valid" action="insert.php" method="POST">
                <div>
         ...
         <script>
            ...
            function check_phone() {
                ...
                if (phone == "") {
                    ...
                    document.getElementById("phone_alert").innerHTML = "Phone number must contain 10 digits";
                    document.Valid.phone.focus();
                    return false;
                }
