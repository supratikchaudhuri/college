...
#include <cuda.h>
 ...
 #include <time.h>
 #define SIZE 100
 __global__ void max(int *a , int *c) // kernel function
definition
 {
 int i = threadIdx.x; //
 ...
 *c = a[i];
 ...
 int main() {
    int i;
    srand(time(NULL)); //makes use of the computer's internal
    clock to control the choice of the seed
    int a[SIZE];
    int c;
    int *dev_a, *dev_c; //GPU / device parameters
    cudaMalloc((void **) &dev_a, SIZE*sizeof(int)); //assign
    memory to parameters on GPU from CUDA runtime API
    cudaMalloc((void **) &dev_c, SIZE*sizeof(int));
